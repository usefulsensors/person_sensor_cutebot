...
class RINGBIT(object):
    ...
    def set_motors_speed(self, left_wheel_speed: float, right_wheel_speed: float):
        """
        设置左右轮电机速度
        ...
        if left_wheel_speed > 0:
            left_value = ((left_wheel_speed - 0) *
                                (256 - 153.6)) / (100 - 0) + 153.6
        elif left_wheel_speed < 0:
            left_value = ((left_wheel_speed - 0) *
                                (51.2 - 153.6)) / (-100 - 0) + 153.6
        self.__left_wheel_pin.write_analog(left_value)

        right_value = 153.6
        right_wheel_speed = right_wheel_speed * -1
        if right_wheel_speed > 0:
            right_value = ((right_wheel_speed - 0)
                                 * (256 - 153.6)) / (100 - 0) + 153.6
        elif right_wheel_speed < 0:
            right_value = ((right_wheel_speed - 0)
                                 * (51.2 - 153.6)) / (-100 - 0) + 153.6
        self.__right_wheel_pin.write_analog(right_value)
